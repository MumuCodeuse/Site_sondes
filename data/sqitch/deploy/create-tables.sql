-- Création des tables pour la base de données sur les sondes spatiales et les rovers
BEGIN;
...
DROP TABLE IF EXISTS probe_scientific_tool CASCADE;
...
DROP TABLE IF EXISTS tl_probe_scientific_tool CASCADE;
...
CREATE TABLE tl_rover_scientific_tool (
    ...
    FOREIGN KEY (rover_id) REFERENCES rover(rover_id) ON DELETE CASCADE,
    PRIMARY KEY (rover_id, rover_scientific_tool_id) -- clé composite
);
...
DROP TABLE IF EXISTS tl_agency_enterprise_country CASCADE;
CREATE TABLE tl_agency_enterprise_country (
    agency_id INT NOT NULL,
...
CREATE INDEX idx_space_probe_name ON space_probe(space_probe_name); -- Recherche par nom de sonde

DROP INDEX IF EXISTS idx_rover_name;
CREATE INDEX idx_rover_name ON rover(rover_name); -- Recherche par nom de rover

DROP INDEX IF EXISTS idx_space_probe_year_launch;
CREATE INDEX idx_space_probe_year_launch ON space_probe(space_probe_year_launch); -- Recherche par année de lancement de sonde
...
-- CREATE INDEX idx_probe_mission_rover ON tl_probe_mission(mission_id, rover_id);--rover associé à une mission spécifique
